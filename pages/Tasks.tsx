            ...
            {(['ALL', TaskPriority.HIGH, TaskPriority.MEDIUM, TaskPriority.LOW] as const).map((f) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-xl text-[10px] font-bold uppercase transition-all ${
                  priorityFilter === f ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-500 hover:text-slate-300'
                }`}
              >
                {f === 'ALL' ? 'Todos' : f}
              </button>
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                      <button onClick={() => handleOpenEditTask(task)} className="p-1 text-slate-500 hover:text-blue-400"><Edit2 size={14} /></button>
                  ...
                  {task.date && (
                    <div className="flex items-center gap-1.5 text-[10px] font-medium text-blue-400 bg-blue-500/10 px-2 py-1 rounded-lg w-fit">
                      <CalendarIcon size={12} /> {new Date(task.date).toLocaleDateString('pt-BR')}
                    </div>
                  )}
                </div>
              ))}
          ...
          <div key={editingTask?.id || 'new-task'} className="w-full max-w-lg glass p-8 rounded-3xl animate-in zoom-in-95 border-white/5 shadow-2xl">
            ...
            <form onSubmit={handleCreateTask} className="space-y-4">
              <div>
                ...
                <input name="title" required defaultValue={editingTask?.title || ''} className="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl outline-none" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">Descrição</label>
                <textarea name="description" defaultValue={editingTask?.description || ''} className="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl h-24 resize-none" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">Urgência</label>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">Data Prazo</label>
              ...
              <div className="flex gap-3 mt-8">
                ...
                <button type="submit" className="flex-1 py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl">Salvar</button>
              </div>
            </form>
          </div>
        </div>
