...
import { 
  ...
  Lightbulb, ExternalLink, Search, MessageSquareText, 
  Cpu, Key
} from 'lucide-react';

const FormattedAIResponse = ({ text }: { text: string }) => {
  if (!text) return null;
  // Limpeza agressiva de qualquer markdown indesejado para manter o estilo executivo
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
      {lines.map((line, index) => {
        ...
        // Formatação de Títulos (### ou Linhas Curtas em CAIXA ALTA)
          ...
          return (
            <div key={index} className="pt-6 pb-2">
              <div className="flex items-center gap-4 mb-2">
                <span className="h-px flex-1 bg-gradient-to-r from-emerald-600/50 to-transparent"></span>
                <h3 className="text-emerald-400 font-black text-[10px] uppercase tracking-[0.4em]">
        ...
        if (trimmed.startsWith('-')) {
          return (
            <div key={index} className="flex gap-4 pl-2 py-1 group">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-600 mt-2 flex-shrink-0 group-hover:scale-150 transition-transform"></div>
...
const AIAgent = () => {
  ...
  const [response, setResponse] = useState<AIResponse>({ text: '', sources: [], provider: 'openai' });
  ...
  const handleConsult = async (customPrompt?: string) => {
    ...
    setPrompt('');
    setLoading(true);
    setResponse({ text: '', sources: [], provider: 'openai' });
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-5">
          <div className="w-14 h-14 bg-gradient-to-br from-emerald-600 to-teal-700 rounded-[22px] flex items-center justify-center shadow-2xl border border-white/10 rotate-3 transition-transform hover:rotate-0">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <p className="text-slate-500 font-black text-[9px] uppercase tracking-[0.2em]">
                {loading ? 'Consultando GPT-4o' : 'Motor ChatGPT Online'}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-1 p-1.5 bg-slate-900 rounded-2xl border border-slate-800 shadow-inner">
            {(['commercial', 'persuasive'] as const).map((s) => (
              <button
                key={s}
                onClick={() => setStyle(s)}
                className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${style === s ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
              >
          ...
          <div className="glass p-8 rounded-[32px] border-white/5 shadow-2xl space-y-8 bg-slate-900/40">
            <div>
              <h4 className="font-black text-[10px] text-slate-500 uppercase tracking-[0.3em] mb-6 flex items-center gap-2">
                <Lightbulb size={14} className="text-amber-500" /> Consultoria Rápida
              </h4>
              <div className="space-y-3">
                {[
                  { label: "Análise de ROI", p: "Analise meu pipeline de empresas e sugira como aumentar o ROI das parcerias." },
                  { label: "Plano de Expansão", p: "Gere um roteiro tático para expansão em novas IES este semestre." }
                ].map((item, i) => (
                  <button key={i} onClick={() => handleConsult(item.p)} className="w-full text-left p-4 rounded-2xl bg-slate-950/40 border border-slate-800/50 hover:border-emerald-500/50 hover:bg-slate-900 transition-all group">
          ...
          <div className="glass rounded-[40px] border-white/5 shadow-2xl flex flex-col h-[700px] relative overflow-hidden bg-slate-950/40">
            <div className="px-10 py-5 border-b border-slate-800/60 flex items-center justify-between bg-slate-950/40">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">
                  Inteligência Estratégica ChatGPT
                </span>
              </div>
              {response.text && (
                <button 
                  onClick={() => { navigator.clipboard.writeText(response.text); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
                  className="flex items-center gap-2 px-4 py-2 hover:bg-slate-800 rounded-xl text-slate-500 hover:text-emerald-400 transition-all border border-slate-800"
                >
            ...
            <div ref={scrollRef} className="flex-1 p-12 overflow-y-auto custom-scrollbar">
              {loading && !response.text ? (
                <div className="h-full flex flex-col items-center justify-center space-y-6">
                  ...
                  <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] animate-pulse">Sincronizando com OpenAI...</p>
                </div>
              ) : response.text?.includes('CHAVE OPENAI INVÁLIDA') ? (
                <div className="h-full flex flex-col items-center justify-center max-w-md mx-auto text-center space-y-8 animate-in slide-in-from-bottom-4">
                  ...
                  <div className="space-y-4">
                    <h3 className="text-2xl font-black text-white italic uppercase tracking-tight">Falha Técnica</h3>
                    <p className="text-slate-400 text-[13px] leading-relaxed font-medium">
                      A chave de API configurada não é válida para a OpenAI.
                    </p>
                  </div>
                  <div className="p-6 bg-slate-900/60 rounded-2xl border border-slate-800 text-left">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Checklist de Solução:</p>
                    <ul className="text-[11px] text-slate-400 space-y-3">
                      <li className="flex gap-2">
                        <span className="text-emerald-500 font-bold">1.</span> 
                        Certifique-se que sua chave começa com <code className="text-emerald-400 font-bold">sk-...</code>
                      </li>
                      <li className="flex gap-2">
                        <span className="text-emerald-500 font-bold">2.</span> 
                        Verifique se você possui créditos na conta OpenAI Platform.
                      </li>
                    </ul>
                  </div>
                </div>
            ...
            <div className="p-8 bg-slate-950/80 border-t border-slate-800/60 backdrop-blur-md">
              <form 
                ...
                className="flex items-center gap-4 bg-slate-900/50 p-2 rounded-[24px] border border-slate-800 focus-within:border-emerald-500/50 transition-all shadow-inner"
              >
                <input 
                  value={prompt}
                  onChange={(e) => setPrompt(e.target.value)}
                  placeholder="Solicite uma análise diagnóstica para suas parcerias..."
                ...
                <button 
                  ...
                  type="submit"
                  className="px-8 py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-[18px] transition-all shadow-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2"
                >
